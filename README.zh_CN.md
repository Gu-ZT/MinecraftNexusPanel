...
当前 M2 领域模型和模板目录已覆盖 27 类公开服务端、代理端与面向基岩版的运行画像：
...
| Java 混合端 | Mohist、Magma、Sponge、Arclight、Youer、Silkard、CatServer |
...
模板目录与已验证安装器是两个层次：当前已为全部 27 类公开类型接入版本元数据提供方，包括 Vanilla、Paper、Velocity、Fabric、NeoForge、Forge、Bukkit、Spigot、Purpur、Pufferfish、Folia、Leaf、Mohist、Youer、Silkard、Magma、Sponge、Arclight、CatServer、Waterfall、BungeeCord、Lightfall、Geyser、Bedrock Dedicated Server、PocketMine-MP、Nukkit、Cloudburst Nukkit。NeoForge 使用官方 Maven XML 目录，Pufferfish 聚合五个官方 Jenkins job，Bukkit 和 Spigot 使用官方 Jenkins RSS Atom feed，Mohist 和 Youer 使用 MohistMC 官方 project API，Silkard 使用官方 GitHub branches API，GitHub provider 要求存在 JAR/PHAR 资产，BDS 使用 Mojang 官方下载链接 API 解析 Windows/Linux 稳定版和 Preview ZIP，Nukkit 变体使用 OpenCollab Maven 元数据；Sponge 当前对应官方历史 SpongeVanilla Releases，Magma 主要提供开发构建。NeoForge 是首个完成验证的安装配方：用户选择 Minecraft 版本、对应 loader 版本和 Java runtime，Panel 解析官方 installer 的大小与 SHA-256，Core 使用要求的 Java 主版本执行 installer、校验生成的平台参数文件、写入 `user_jvm_args.txt`，并仅在安装成功后注册实例。其余 26 类的版本元数据仍不代表产物已验证可直接安装，归档结构、启动配方和更新流程仍需按类型与版本逐项补充。
...
提供生命周期操作、筛选分页和 Core/服务端类型/实例参数分步创建向导的实例卡片目录；向导会按
Java、原生基岩端、PocketMine-MP 或自定义运行时切换启动字段。其他页面包括可检查 CPU 拓扑的只读节点目录、按权限显示的用户管理、本地客户端设置，以及
...
增量 SHA-256 与分块上传/下载，以及带任务轮询的异步递归删除。Core 编辑、镜像和 Panel 全局设置
等后端尚无授权 API 的能力不会以无效按钮提前出现。
