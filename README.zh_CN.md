...
- Panel 已支持按模板和 `PLUGIN`/`MOD` 类型扫描扩展：每个声明目录独立返回文件页，缺失目录返回空页；还可通过受限原子写入放置已准备的本地产物，并在显式确认、校验幂等键后异步删除单个文件。共享 TypeScript Client 已暴露这些操作和基岩扩展目录字段。扩展安装记录、来源与依赖解析、兼容性校验以及完整安装/更新流程仍待完成。
