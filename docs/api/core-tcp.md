...
当前 Core 已实现 `PROPERTIES` 和 `JSON` 提供者：`config.scan` 递归发现 `.properties` 与 `.json` 文件，`config.get` 返回 JSON Schema、UI Schema、结构化值、未映射文本、内容
SHA-256 `revision`/`contentHash` 和稳定的路径派生 `documentId`。`config.patch` 要求幂等键和当前 revision，并通过原子文件替换；properties 补丁只接受顶层字符串、布尔、数字或 `null`，保留注释、键顺序和换行，JSON 补丁支持嵌套 Merge Patch 但只有显式 `allowLossy=true` 才允许规范化写回。Panel 可通过对应 raw 端点读取或替换最多 1 MiB 的 UTF-8 原文。
YAML、TOML、provider-specific Schema 和跨文件校验仍需后续 provider。
