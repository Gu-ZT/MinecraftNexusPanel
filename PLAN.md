...
- 当前文件管理首阶段通过 `files` capability 暴露 Core 沙箱列表、32 KiB 分块读取和 1 MiB 内原子写入；Panel REST 使用二进制响应、`ETag`/`If-Match` 和幂等键传递同一约束。
...
- 已交付能力包括 `file.list` 分页、`file.read` 分块读取和 `file.write` 小文件原子替换。读取上限为 32 KiB，整体写入上限为 1 MiB，写入可用完整文件 SHA-256 做乐观并发校验。
- 目录创建和同一实例内移动已支持递归目录、覆盖选项和非空目录保护；删除、批量操作和大文件传输仍需通过 Core 临时文件、配额和任务中心实现，不能把小文件 PUT 当作大文件上传协议。
...
| 文件管理  | 目录列表、分块读取、原子写入、目录创建、移动、ETag/If-Match | 删除、批量操作、分块上传下载、快照与差异比较 |
...
- 配置识别和结构化表单、文件管理、分块上传、实例终端；当前先完成文件沙箱列表、分块读取和小文件原子写入，再补齐目录与任务化传输。
