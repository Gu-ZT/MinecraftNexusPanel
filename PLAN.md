...
| 一键搭建  | 27 类服务端/代理/基岩模板目录与版本目录；NeoForge 版本化计划解析、可信下载和异步安装 | 其余类型的版本化安装配方、模板市场、自定义签名模板、整包导入 |
