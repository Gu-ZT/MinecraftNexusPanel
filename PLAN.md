...
- **Panel 用户级审计**：Panel HTTP 中间件持久化用户 ID、请求 ID、来源 IP、HTTP 方法、无查询参数路径、状态码和权限结果；管理员及显式持有 `audit.read` 的非管理员可通过 `GET /api/v1/audit-events` 按最新优先读取最近 10,000 条。管理员可通过 `/api/v1/users` 列出、创建、查询、更新和删除非管理员；权限变更对现有会话立即生效，删除会级联撤销会话，管理员账户和自删除受保护。目前只允许授予已执行服务端检查的 `audit.read`，未知权限拒绝写入。请求体、查询参数、Cookie、Token 和密码不落库。用户组、资源范围筛选、导出和可配置保留策略仍属于后续阶段。
