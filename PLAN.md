...
- **Panel 用户级审计**：Panel HTTP 中间件持久化用户 ID、请求 ID、来源 IP、HTTP 方法、无查询参数路径、状态码和权限结果；管理员及显式持有 `audit.read` 的非管理员可通过 `GET /api/v1/audit-events` 按最新优先读取保留窗口，也可通过 `/api/v1/audit-events:export` 按相同安全字段导出 NDJSON。保留数量默认 10,000，可通过 `MCNP_AUDIT_RETENTION_EVENTS` 或 `--panel-audit-retention-events` 在 `100..=100000` 内设置，写入与裁剪使用同一 SQLite 事务，导出使用存储实例的相同上限。管理员可通过 `/api/v1/users` 列出、创建、查询、更新和删除非管理员；权限变更对现有会话立即生效，删除会级联撤销会话，管理员账户和自删除受保护。目前只允许授予已执行服务端检查的 `audit.read`，未知权限拒绝写入。请求体、查询参数、Cookie、Token 和密码不落库。用户组和资源范围筛选仍属于后续阶段。
...
- Tauri Desktop：已交付 Windows x64 独立安装包；安装包包含共享 Vue 构建产物和 release `mcnp all` sidecar。首启自动生成 Panel 主密钥、Core PSK 和随机管理员密码，并自动换取原生会话，无需用户手动登录；Panel 仅对 Tauri 本地来源开放跨源请求，退出时停止 sidecar。
- Tauri Desktop 已提供显示动态 Panel 地址的系统托盘、关闭到托盘、主窗口恢复、显式退出、单实例重复启动唤醒、sidecar stdout/stderr 逐行 JSON 日志收集、结构化秘密字段遮盖和日志目录入口、Windows Credential Manager/macOS Keychain/Linux Secret Service Refresh Token 存储、仅限 loopback 和设备秘密的自动原生会话引导，以及可在设置页管理的当前用户开机启动；登录项启动时静默驻留托盘。后续补齐日志文件加密、签名和自动更新。
