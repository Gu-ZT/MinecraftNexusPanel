...
- Core records instance start success/failure, stop and force-kill requests, and unexpected supervised-process exits. MCDR wrapper exits receive a dedicated failure reason, and the newest records can be queried through `instance.audit.list`, Panel REST, and the shared TypeScript Client. Core persists the latest 2,048 records in `instance-audit.json` with atomic replacement and refuses to start when that file is corrupted. Panel also persists user-level request audit events through `GET /api/v1/audit-events` without request bodies, query parameters, cookies, tokens, or passwords; the same retained safe fields can be exported as NDJSON. Retention defaults to 10,000 events and can be configured from 100 to 100,000 through `MCNP_AUDIT_RETENTION_EVENTS` or the matching CLI option. Administrators can create, list, update, and delete non-administrator users; permission changes affect existing sessions immediately and deletion revokes sessions. An explicit durable `audit.read` grant allows audit access without administrator privileges. Groups and resource scopes remain planned.
...
- Windows Core now reads Processor Relationship records across processor groups and maps distinct
  `EfficiencyClass` extremes to performance and efficiency classes. Intermediate levels and
  unreported isolation or NUMA data remain explicit unknown values.
...
Equivalent topology discovery on other platforms, actual CPU affinity/cpuset application, and
cross-Core scheduling locks remain planned M4 work.
