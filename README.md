...
- Panel can now scan extensions by template and `PLUGIN`/`MOD` kind, return each declared directory as a separate file page, place a prepared local artifact through a bounded atomic write, and asynchronously delete one file after explicit confirmation and idempotency validation. Installation records, source and dependency resolution, compatibility checks, and complete installation/update flows remain planned.
