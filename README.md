...
- Panel can now scan extensions by template and `PLUGIN`/`MOD` kind, return each declared directory as a separate file page, place or update a prepared local artifact through bounded atomic writes with optional `If-Match`, persist its local installation metadata, and asynchronously delete one file after explicit confirmation and idempotency validation. The shared TypeScript Client exposes these operations and Bedrock extension directories. Source and dependency resolution, compatibility checks, and complete installation/update flows remain planned.
