...
- Panel can now scan extensions by template and `PLUGIN`/`MOD` kind, returning each declared directory as a separate file page and treating missing directories as empty pages. It can also asynchronously delete one file within the selected template layout after explicit confirmation and idempotency validation. Installation records, installation, update, compatibility resolution, and source search remain planned.
