...
- Panel can now scan extensions by template and `PLUGIN`/`MOD` kind, return each declared directory as a separate file page, place or update a prepared local artifact through bounded atomic writes with optional `If-Match`, persist its local installation metadata, and asynchronously delete one file after explicit confirmation and idempotency validation. The extension catalog also searches Modrinth MOD/PLUGIN projects with Minecraft version, loader, and pagination filters, returning source metadata compatibility hints. The shared TypeScript Client exposes these operations and Bedrock extension directories. Dependency resolution, additional source adapters, and complete installation/update flows remain planned.
