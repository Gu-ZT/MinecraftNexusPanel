...
- Core records instance start success/failure, stop and force-kill requests, and unexpected supervised-process exits. MCDR wrapper exits receive a dedicated failure reason, and the newest records can be queried through `instance.audit.list`, Panel REST, and the shared TypeScript Client. Core persists the latest 2,048 records in `instance-audit.json` with atomic replacement and refuses to start when that file is corrupted. Panel also persists user-level request audit events through `GET /api/v1/audit-events`, retaining at most 10,000 records without request bodies, query parameters, cookies, tokens, or passwords; the same retained safe fields can be exported as NDJSON. Administrators can create, list, update, and delete non-administrator users; permission changes affect existing sessions immediately and deletion revokes sessions. An explicit durable `audit.read` grant allows audit access without administrator privileges. Groups, resource scopes, and configurable retention remain planned.
