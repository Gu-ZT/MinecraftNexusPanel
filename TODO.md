...
- [x] 前端 v2 设计迁移：按设计规范分批重写外壳与各页面（详见 M5 前端/客户端泳道）。
...
- [ ] CPU 拓扑识别：物理核、逻辑 CPU、NUMA、性能/能效类别。
  ...
  - [x] Windows Processor Relationship/EfficiencyClass 探测，支持处理器组、物理核心和保守的性能/能效层级分类。
  - [ ] macOS、BSD 和其他平台等价探测。
...
- [ ] 完成 WebUI 全部管理页面和权限驱动交互。
  ...
  - [x] 实例目录提供 Core、服务端类型、类型参数三步创建向导；路由已选中有效 Core 时跳过第一步，Java、原生基岩端、PocketMine-MP 和自定义运行时分别展示对应启动字段，创建成功后直接进入实例工作区。
...
- [x] 前端 v2 现代仪表盘风格迁移（依据 [docs/design/frontend-design.md](docs/design/frontend-design.md)，路由、store、API Client 与平台适配器契约不变）。
  - [x] 设计令牌 v2：替换 `base.css` 令牌为卡片化/玻璃拟态/渐变强调体系，桥接 Arco 变量，保留双主题与 `prefers-reduced-motion`。
  - [x] 应用外壳：玻璃拟态侧边导航 + 顶栏 + 内容区布局，移动端底部标签栏；导航权限过滤保持现状。
  - [x] 仪表盘：KPI 统计卡、实例状态分布、节点健康、最近审计动态卡片与骨架屏。
  - [x] 实例目录与创建向导：卡片网格、筛选工具条、模态化三步向导。
  - [x] 实例工作区：实例头卡 + 子页签外壳，概览/终端/配置/文件四视图容器规范。
  - [x] 节点、用户、设置页面卡片化；每批迁移完成后回归既有交互（创建向导、终端刷新、配置保存、文件传输）。
